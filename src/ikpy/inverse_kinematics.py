...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None, max_iter=None):
    ...
    def optimize_target(x):
        ...
        y = chain.active_to_full_joints(x, starting_nodes_angles)
    ...
    real_bounds = chain.active_from_full_joints(real_bounds)
    ...
    res = scipy.optimize.minimize(optimize_total, chain.active_from_full_joints(starting_nodes_angles), method='L-BFGS-B', bounds=real_bounds, options=options)

    print("Inverse kinematic optimisation OK, done in {} iterations".format(res.nit))
    if chain.first_active_joint == 0:
        return(res.x)
    else:
        return(chain.active_to_full_joints(res.x, starting_nodes_angles))
